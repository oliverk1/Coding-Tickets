...
def getWindSpeed(weather):
    windspeedmph = float(weather["windspeed"])
    if windspeedmph < 1:
        windspeed = "Calm"
...
def printWeather(weather, windspeed, direction, heat, location, preprob):
    print("Today the weather in", location, "is", heat.lower(), "with a", windspeed.lower() + ".",
        "\nTemperature:", str(weather["temperature"]) + "℃"
        "\nChance of Precipitation:", str(preprob),
        "\nWindspeed:", str(weather["windspeed"]), "mph"
